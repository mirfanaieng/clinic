  ...
  return (
    <group ref={group}>
      ...
      <mesh castShadow position={[0, 0.1, 0]}>
        ...
        {/* Deliberately not MeshTransmissionMaterial. Fully transmissive glass
            only reads against contrast: over a transparent canvas the buffer is
            black (bottle = silhouette), and forcing the buffer to the card
            colour makes it vanish. Partial transmission keeps the material's
            own tinted shading, which is what gives each bottle its identity on
            an ivory card. */}
        <meshPhysicalMaterial
          ...
          envMapIntensity={1.3}
          attenuationColor={product.glass}
      ...
      <mesh position={[0, 0.1, 0]}>
        ...
        <meshPhysicalMaterial
          ...
          attenuationColor={product.liquid}
      ...
      <mesh position={[0, product.form === "sunblock" ? 0.68 : 0.8, 0]}>
        <cylinderGeometry args={[0.3, 0.3, 0.09, 48]} />
        <meshStandardMaterial color={product.capMetal} metalness={1} roughness={0.24} />
      </mesh>

      <mesh castShadow position={[0, product.form === "sunblock" ? 0.92 : 1.04, 0]}>
        <cylinderGeometry args={[0.285, 0.3, 0.44, 48]} />
      ...
      <mesh position={[0, product.form === "sunblock" ? 0.92 : 1.04, 0]}>
        <torusGeometry args={[0.288, 0.012, 12, 64]} />
      ...
      {[-0.28, -0.44].map((y) => (
        <mesh key={y} position={[0, y, 0]} rotation={[Math.PI / 2, 0, 0]}>
          <torusGeometry
            args={[product.form === "sunblock" ? 0.79 : 0.65, 0.006, 8, 72]}
          />
          <meshStandardMaterial
            color={product.capMetal}
            metalness={0.95}
